...
def lambda_handler(event, context):
    ...
    for region in regions:
        ...
        # Lista todas as instâncias RDS Postgres Community na região atual
        instances = rds.describe_db_instances()
        
        # Filtra as instâncias que são do tipo Postgres Community
        postgres_instances = [
        ...
        for instance in postgres_instances:
            ...
            print(f"ID da instância RDS Postgres Community: {db_instance_identifier}")
            
            # Verificar se a instância está parada
            if db_instance_status == 'stopped':
                try:
                    # Iniciar a instância RDS Postgres Community
                    response = rds.start_db_instance(DBInstanceIdentifier=db_instance_identifier)
                    print(f"Iniciando a instância RDS Postgres Community {db_instance_identifier}. Resposta: {response}")
            ...
            # Verificar se a instância está em execução
                ...
                try:
                    # Parar a instância RDS Postgres Community
                    response = rds.stop_db_instance(DBInstanceIdentifier=db_instance_identifier)
                    print(f"Parando a instância RDS Postgres Community {db_instance_identifier}. Resposta: {response}")
                except Exception as e:
